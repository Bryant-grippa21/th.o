    ...
    return `
      ...
      : '<div style="width:96px; height:96px; border:1px solid #ccc; display:flex; align-items:center; justify-content:center;">Sin imagen</div>'}
        ...
        <div>
          ...
          <p>SKU: ${product.sku}</p>
          <p>Precio USD: ${formatAmount(product.price)}</p>
          ${state.exchangeRate?.rate_bs_per_usd
      ? `<p>Precio Bs: Bs.S ${formatBsAmount(convertUsdToBs(product.price))}</p>`
      : ''}
          <p>Subtotal item USD: ${formatAmount(Number(product.price || 0) * Number(item.quantity || 0))}</p>
          ${state.exchangeRate?.rate_bs_per_usd
      ? `<p>Subtotal item Bs: Bs.S ${formatBsAmount(convertUsdToBs(Number(product.price || 0) * Number(item.quantity || 0)))}</p>`
      : ''}
