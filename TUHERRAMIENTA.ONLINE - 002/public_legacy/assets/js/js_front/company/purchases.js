...
const renderPaymentMethods = () => {
  if (!state.paymentMethods.length) {
    paymentMethodsList.innerHTML = '<p>Esta empresa aún no tiene métodos de pago registrados.</p>';
    return;
  }
...
const getGroupActionsHtml = (group) => {
  if (group.status === 'PAYMENT_SUBMITTED') {
    return `
      ...
      <textarea data-group-note="${group.id_purchase_group}" rows="3" placeholder="Nota de revisión opcional" style="width:100%; max-width:420px; margin-top:8px;"></textarea>
    `;
  }
  ...
  purchaseGroupsList.innerHTML = filteredGroups.map((group) => `
    <details style="border:1px solid #ccc; padding:16px; margin-bottom:16px;">
      ...
      <div style="margin-top:12px;">
        ...
        <p>Nota de revisión: ${group.review_note || 'Sin observaciones'}</p>
        <details>
...
const renderSummary = () => {
  ...
  summaryElement.innerHTML = `
    ...
    Métodos de pago: <b>${state.paymentMethods.length}</b> |
...
const handlePaymentMethodSubmit = async (event) => {
  ...
  try {
    ...
    setFeedback('Guardando método de pago...');
    ...
    setFeedback('Método de pago registrado correctamente.', 'success');
    await refreshView();
  } catch (error) {
...
const handleGroupAction = async (action, groupId) => {
  ...
  try {
    setFeedback(actionMessages[action] || 'Procesando acción...');
    ...
    setFeedback(successMessages[action] || 'Acción ejecutada correctamente.', 'success');
    await refreshView();
  } catch (error) {
    setFeedback(error.message, 'error');
  }
