...
export default function NaturalUserDashboard({ userData, onBack, onLogout }) {
  ...
  const addToFavorites = (product) => { if (!favorites.some(f => f.id === product.id)) setFavorites([...favorites, product]); };
  const removeFromFavorites = (productId) => setFavorites(favorites.filter(p => p.id !== productId));
  const isFavorite = (productId) => favorites.some(f => f.id === productId);

  const addToCart = (product, quantity = 1) => {
    const existingItem = cart.find(item => item.id === product.id);
    if (existingItem) setCart(cart.map(item => item.id === product.id ? { ...item, quantity: item.quantity + quantity } : item));
      ...
      <header className="sticky top-0 z-50 shadow-sm" style={{ backgroundColor: PALETTE.maastrichtBlue }}>
        <div className="px-4 py-3 lg:px-8">
          <div className="mx-auto flex max-w-7xl items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => setShowCart(!showCart)} className="relative flex items-center gap-2 rounded-full px-3 py-2 text-white hover:bg-white/10"><ShoppingBag className="h-5 w-5" />{cartQuantity > 0 && (<span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full w-5 h-5 flex items-center justify-center">{cartQuantity}</span>)}</button>
