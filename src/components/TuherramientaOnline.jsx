      ...
      >
        ...
        <CardContent className="space-y-3 p-4">
          ...
          <div className="space-y-1">
            ...
            {/* Cashback por unidad - Solo visible para usuarios naturales */}
            {sessionType === "natural" && (
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Cashback del producto - Solo visible para usuarios naturales */}
        {sessionType === "natural" && (
          <div className="flex items-center justify-between">
...
export default function TuherramientaOnlineAmazonUI() {
  ...
  const [sessionType, setSessionType] = useState("juridico_detallista");
  ...
  // Actualizar datos del usuario según el tipo de sesión
  useEffect(() => {
    const getUserData = () => {
      switch (sessionType) {
        case "natural":
  ...
  // ==========================================================
  // FILTRADO DE PRODUCTOS - CORREGIDO PARA MAYORISTAS
  // ==========================================================
  const filteredProducts = useMemo(() => {
    // Para mayoristas: solo ver sus propios productos
    ...
    // Para el resto de usuarios: filtros normales
    return products.filter((product) => {
  ...
  const canBuyProduct = (product) => {
    // Invitados no pueden comprar
    if (sessionType === "guest") return false;
    
    // Mayoristas no pueden comprar
    if (sessionType === "juridico_mayorista") return false;
    
    // Naturales solo pueden comprar de DETALLISTAS
    if (sessionType === "natural") {
      return product.sellerRole === "DETALLISTA";
    }
    
    // Detallistas solo pueden comprar de MAYORISTAS
  ...
  const handleCheckout = () => {
    if (sessionType === "guest") {
      ...
      alert("✅ Procesando pago...\n\nTotal: " + formatUSD(cartSummary.subtotal));
    }
  };
  ...
  const addToCart = (product) => {
    ...
    // Verificar si puede comprar el producto (para naturales y detallistas)
  ...
  // Mostrar página de tienda completa (marketplace)
  if (showFullStore) {
    return (
      <Marketplace 
        onBack={() => {
    ...
    >
            ...
            <div className="hidden items-center gap-4 md:flex">
              {sessionType !== "guest" ? (
                ...
                <>
                  ...
                  {/* Tarjeta de Cashback - Solo visible para usuarios naturales */}
                  {sessionType === "natural" && (
                    <div
                  ...
                  {/* Botón Tienda - Visible para todos los usuarios logueados */}
                  <button
                    ...
                    {isCartOpen && (
                      ...
                      >
                        ...
                        >
                          ...
                          {/* Cashback total - Solo visible para usuarios naturales */}
                          {sessionType === "natural" && (
                            <div className="mt-2 flex items-center justify-between text-sm">
                ...
                // Usuario invitado - mostrar botones de login
                <>
                  <div
                    className="rounded-2xl px-3 py-2"
                    style={{ backgroundColor: "rgba(255,255,255,0.04)" }}
                  ...
                  {/* Botón Tienda - También visible para invitados */}
                  <button
                    onClick={() => setShowFullStore(true)}
                    className="rounded-2xl px-3 py-2 transition-all hover:scale-105 hover:bg-white/10"
                    style={{ backgroundColor: "rgba(255,255,255,0.04)" }}
      ...
      <main className="w-full px-0 py-0">
        <section className="w-full px-0 py-0 lg:px-0">
          {/* --------------------------------------------------
              HERO PRINCIPAL - PEGADO A LOS BORDES
             -------------------------------------------------- */}
          <div
          ...
          {/* --------------------------------------------------
              BLOQUES DESTACADOS
             -------------------------------------------------- */}
          <div className="relative z-20 grid gap-6 md:grid-cols-2 xl:grid-cols-4 px-4 lg:px-8" style={{ marginTop: "-60px" }}>
            <Card
              className="rounded-[2rem] border-0 shadow-lg hover:shadow-xl transition-shadow"
              style={{ backgroundColor: PALETTE.white }}
            >
        ...
        {/* --------------------------------------------------
           ...
           -------------------------------------------------- */}
        <section className="mt-8 grid gap-4 px-4 pb-6 sm:grid-cols-2 xl:grid-cols-4 lg:px-8">
          {categories.map((category) => (
            <motion.button
              key={category.id}
              whileHover={{ y: -4 }}
              onClick={() => setSelectedCategory(category.name)}
              className="overflow-hidden rounded-[2rem] text-left shadow-sm"
              style={{ backgroundColor: PALETTE.white }}
            >
              ...
              <div className="space-y-2 p-4">
                <h3 className="text-lg font-bold" style={{ color: PALETTE.maastrichtBlue }}>{category.name}</h3>
                <p className="text-sm" style={{ color: PALETTE.spaceCadet }}>{category.description}</p>
                <div className="flex flex-wrap gap-2">
                  {category.subcategories.map((sub) => (
                    <Badge key={sub} variant="outline" className="rounded-full" style={{ borderColor: PALETTE.crystalBlue, color: PALETTE.spaceCadet }}>
                      {sub}
                    </Badge>
                  ))}
                </div>
        ...
        {/* --------------------------------------------------
            SECCION PRINCIPAL DE 3 COLUMNAS
           -------------------------------------------------- */}
        <section className="grid gap-6 px-4 pb-10 lg:grid-cols-[260px_1fr_320px] lg:px-8">
          {/* COLUMNA IZQUIERDA: FILTROS */}
          <aside
            className="space-y-4 rounded-[2rem] p-5 shadow-sm"
            style={{ backgroundColor: PALETTE.white }}
          >
            <div>
              <h3 className="text-lg font-bold" style={{ color: PALETTE.maastrichtBlue }}>Explorar</h3>
              <p className="text-sm" style={{ color: PALETTE.crystalBlue }}>Filtros tipo marketplace</p>
            </div>
            ...
            <div className="rounded-2xl p-4" style={{ backgroundColor: PALETTE.softCard }}>
              <p className="text-sm" style={{ color: PALETTE.crystalBlue }}>Condiciones de uso</p>
              <ul className="mt-3 space-y-2 text-sm" style={{ color: PALETTE.spaceCadet }}>
                <li>• Debes estar registrado para poder comprar</li>
                <li>• El cashback se calcula por cada producto del carrito</li>
                <li>• Puedes aumentar, disminuir o eliminar productos antes del pago</li>
                <li>• Los precios pueden variar según disponibilidad</li>
                <li>• Las compras están sujetas a validación del sistema</li>
              </ul>
            </div>
          </aside>

          {/* COLUMNA CENTRAL: RESULTADOS - SOLO 3 PRODUCTOS CON BOTÓN "VER TODOS" */}
          <section>
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-2xl font-black" style={{ color: PALETTE.maastrichtBlue }}>
                  Resultados
                </h2>
              ...
              <motion.button
                onClick={() => setShowFullStore(true)}
                className="rounded-full px-6 py-2 text-sm font-semibold flex items-center gap-2"
                whileHover={{ scale: 1.05, x: 5 }}
                whileTap={{ scale: 0.95 }}
                style={{
                  backgroundColor: "transparent",
                  border: `2px solid ${PALETTE.sizzlingSunrise}`,
            ...
            {filteredProducts.length === 0 ? (
              <Card
                className="rounded-[2rem] border-0 shadow-sm"
                style={{ backgroundColor: PALETTE.white }}
              >
                <CardContent className="flex flex-col items-center justify-center gap-3 p-10 text-center">
                  <EyeOff className="h-10 w-10" style={{ color: PALETTE.crystalBlue }} />
                  <h3 className="text-xl font-bold" style={{ color: PALETTE.maastrichtBlue }}>
                    No hay productos visibles para este perfil
                  </h3>
                  <p className="max-w-lg text-sm" style={{ color: PALETTE.spaceCadet }}>
                    {sessionType === "guest"
                      ? "Regístrate o inicia sesión para desbloquear el catálogo que te corresponde."
                      : sessionType === "juridico_mayorista"
                        ? "No tienes productos registrados en tu tienda."
                        : "No hay coincidencias con los filtros actuales o no existen productos del rol permitido."}
                  </p>
                </CardContent>
              </Card>
            ) : (
              <>
                ...
                {/* Botón Ver todos los productos - Solo aparece si hay más de 3 productos */}
          ...
          <aside
            className="rounded-[2rem] p-5 shadow-sm flex flex-col"
            style={{ backgroundColor: PALETTE.white }}
          >
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm" style={{ color: PALETTE.crystalBlue }}>Resumen</p>
                <h3 className="text-xl font-bold" style={{ color: PALETTE.maastrichtBlue }}>Carrito</h3>
              </div>
              <Package className="h-5 w-5" style={{ color: PALETTE.crystalBlue }} />
            </div>

            {!accessProfile.canUseCart && (
              <div className="rounded-2xl p-3 text-sm mt-3" style={{ backgroundColor: PALETTE.softCard, color: PALETTE.spaceCadet }}>
                Este perfil no puede usar carrito. El flujo correcto aquí es autenticación o panel de proveedor.
              </div>
            )}

            <div className="space-y-3 overflow-y-auto mt-3" style={{ maxHeight: "450px" }}>
              {cart.length === 0 ? (
                <div className="rounded-2xl p-3 text-sm" style={{ backgroundColor: PALETTE.softCard, color: PALETTE.spaceCadet }}>
                  Tu carrito está vacío. Añade productos para simular compra y checkout.
                </div>
              ) : (
                cart.map((item) => (
                  <CartItem
            ...
            <div className="rounded-[1.7rem] p-4 text-white mt-3" style={{ backgroundColor: PALETTE.maastrichtBlue }}>
              <div className="flex items-center justify-between text-sm">
                <span style={{ color: PALETTE.pastelGray }}>Subtotal</span>
                <span>{formatUSD(cartSummary.subtotal)}</span>
              </div>
              <div className="mt-2 flex items-center justify-between text-sm">
                <span style={{ color: PALETTE.pastelGray }}>Subtotal Bs</span>
                <span>{formatVES(cartSummary.subtotal)}</span>
              </div>
              {sessionType === "natural" && (
                <div className="mt-2 flex items-center justify-between text-sm">
                  <span style={{ color: PALETTE.pastelGray }}>Cashback total</span>
                  <span className="font-semibold" style={{ color: PALETTE.sizzlingSunrise }}>
                    {formatUSD(cartSummary.cashback)}
                  </span>
                </div>
              )}
              <Button
                className="mt-4 w-full rounded-full font-semibold disabled:cursor-not-allowed disabled:opacity-50"
                style={{ backgroundColor: PALETTE.sizzlingSunrise, color: PALETTE.maastrichtBlue }}
                onClick={handleCheckout}
                disabled={cart.length === 0 || sessionType === "juridico_mayorista"}
              >
            ...
            {(sessionType === "juridico_detallista" || sessionType === "juridico_mayorista") && (
              <div className="rounded-2xl p-3 text-sm mt-3" style={{ backgroundColor: "#EAF4F8", color: PALETTE.spaceCadet }}>
                <p className="font-semibold" style={{ color: PALETTE.maastrichtBlue }}>Bloque jurídico sugerido</p>
                <p className="mt-1">Aquí puedes colocar crédito disponible, pagos recientes y alertas de vencimiento.</p>
              </div>
            )}
          </aside>
        </section>
