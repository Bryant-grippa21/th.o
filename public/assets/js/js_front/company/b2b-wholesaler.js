...
const renderDetailInto = (quoteId, containerId) => {
  ...
  container.innerHTML = `
    <details open>
      ...
      <p>Cargos:</p>
      <ul>${charges.map((charge) => `<li>${charge.charge_type}: USD ${formatMoney(charge.amount_usd)} ${charge.label ? `(${charge.label})` : ''}</li>`).join('') || '<li>Sin cargos</li>'}</ul>
      <p>Evidencias:</p>
  ...
  if (quote.status === 'REQUESTED' || quote.status === 'QUOTED') {
    return `
      <details style="margin-top:8px;">
        <summary>Responder cotizacion</summary>
        ...
        <div style="margin-top:8px;">
          <button type="button" data-quote-action="respond" data-quote-id="${quoteId}">Enviar cotizacion</button>
        </div>
      </details>
    `;
  }
  ...
  if (quote.status === 'PAYMENT_SUBMITTED') {
    return `
      <div style="margin-top:8px;">
        ...
        <input id="quote-note-${quoteId}" type="text" placeholder="Nota de revision" style="width:100%; max-width:760px; margin-top:8px;">
...
quotesList.addEventListener('click', async (event) => {
  ...
  const quoteId = Number(button.getAttribute('data-quote-id'));
  ...
  try {
    ...
    if (action === 'respond') {
      ...
      setFeedback('Cotizacion respondida.', 'success');
      await refreshData();
      await loadQuoteDetail(quoteId);
      return;
    }
...
fetchCurrentSession()
  .then(async (session) => {
    ...
    if (roleId !== ROLE_WHOLESALER) {
      throw new Error('Solo mayoristas pueden usar este modulo');
    }

    if (company.verification_status !== 'APPROVED' || !company.can_sell) {
      throw new Error('Tu empresa no esta habilitada para cotizaciones B2B como mayorista');
    }
