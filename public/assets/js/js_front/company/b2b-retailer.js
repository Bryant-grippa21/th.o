...
const quoteActionsHtml = (quote) => {
  ...
  if (quote.status === 'DELIVERED') {
    return `
      <div style="display:flex; gap:8px; flex-wrap:wrap; margin-top:8px;">
        <button type="button" data-quote-action="confirm-delivery" data-quote-id="${quoteId}">Confirmar recepción</button>
      </div>
      <input id="quote-note-${quoteId}" type="text" placeholder="Nota opcional" style="width:100%; max-width:480px; margin-top:8px;">
    `;
  }
...
const handleDraftAction = async (button) => {
  ...
  if (action === 'submit') {
    if (!confirm(`¿Enviar borrador #${draftId} como solicitud de cotización?`)) {
      return;
    }
...
const handleQuoteAction = async (button) => {
  ...
  if (action === 'confirm-delivery') {
    if (!confirm('¿Confirmar recepción de la mercancía?')) {
      return;
    }
    ...
    setFeedback('Recepción confirmada.', 'success');
    await refreshData();
    return;
  }
