...
const renderVerificationDocuments = (documents = []) => {
  ...
  companyVerificationDocumentsList.innerHTML = Array.from(documentsByType.entries()).map(([documentType, roundsMap]) => {
    ...
    return `
      <details class="company-verification-type" open>
        <summary><b>${getCompanyDocumentLabel(documentType)}</b> (${rounds.reduce((total, [, files]) => total + files.length, 0)} archivos)</summary>
        <div class="company-verification-type__content">
          ${rounds.map(([round, files], index) => `
            <details class="company-verification-round" ${index === 0 ? 'open' : ''}>
              <summary><b>Ronda ${round}</b> (${files.length} archivo${files.length === 1 ? '' : 's'})</summary>
              <div class="company-verification-round__content">
                ${files.map((document) => `
                  <article class="company-verification-file">
...
const renderVerificationHistory = (history = []) => {
  ...
  companyVerificationHistoryList.innerHTML = `
    ...
    ${visibleEntries.map((entry) => `
      <article class="company-verification-history-item">
    ...
    `).join('')}
    <div class="company-verification-history-actions">
...
const renderCompanyImage = (company) => {
  ...
  if (!company?.img_profile) {
    ...
    companyImagePreview.innerHTML = `
      <div class="company-profile-avatar company-profile-avatar--fallback">
        ${fallbackLetter}
      </div>
    `;
    return;
  ...
  companyImagePreview.innerHTML = `
    <img class="company-profile-avatar" src="/uploads/profiles/companies/${company.img_profile}" alt="Imagen de empresa">
  `;
};
