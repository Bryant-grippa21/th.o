...
const renderCashback = (data) => {
  ...
  if (!history.length) {
    cashbackHistory.innerHTML = '<p>Aún no tienes movimientos de cashback.</p>';
    return;
  }

  cashbackHistory.innerHTML = `<div class="customer-card-list">${history.map((entry) => {
    ...
    let subtotalLabel = '';
    if (purchaseItem) {
      subtotalLabel = ` | Base USD ${formatAmount(purchaseItem.subtotal_usd)}`;
    } else if (purchaseCheckout) {
      subtotalLabel = ` | Total orden USD ${formatAmount(purchaseCheckout.total_usd)}`;
    } else if (purchaseGroup) {
      subtotalLabel = ` | Base grupo USD ${formatAmount(purchaseGroup.subtotal_usd)}`;
    }

    return `
      <article class="customer-card">
        <div class="customer-card__body">
          <p class="customer-card__title"><b>${getTransactionLabel(entry.transaction_type)}</b></p>
          <p class="customer-card__meta">Valor: USD ${formatAmount(entry.value)}</p>
          <p class="customer-card__meta">Origen: ${originLabel}${subtotalLabel}</p>
          <p class="customer-card__meta">Orden: ${purchaseCheckout?.order_code || purchaseGroup?.id_checkout || purchaseItem?.id_purchase_group || 'N/D'}</p>
          <p class="customer-card__meta">Fecha: ${formatDateTime(entry.created_at)}</p>
        </div>
      </article>
    `;
  }).join('')}</div>`;
